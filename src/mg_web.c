...
Version 3.1.50 5 July 2025:
   Report an error message in the event log if a response generated by YottaDB is too large to fit into the available buffer space.
   - The message will recommend configuring mg_web to allocate more memory for servicing requests. 
   - This only applies to API-based connectivity to YottaDB.
   - Example:  
...
*/
      ...
      hsize = 0;
      if (out.length > 4) {
         hsize = mg_get_size((unsigned char *) out.address);
      }
      if ((hsize + 5) > out.length) {
         sprintf(buffer, "Insufficient buffer space (response size: %lu; buffer space: %lu). Increase the 'request_buffer_size' in the configuration", (hsize + 5), out.length);
         mg_log_event(pweb->plog, pweb, buffer, "mg_web_execute: YottaDB API error", 0);

         mg_system.request_buffer_size = (hsize + DBX_HEADER_SIZE + DBX_HEADER_MARGIN);
      }

/*
{
   char bufferx[1024];
   sprintf(bufferx, "mg_web_execute (YottaDB:ydb_string_t): rc=%d; fun=%s; len_used=%d; len_alloc=%d; out.length=%d; actual_size=%lu", rc, fun.label, pweb->output_val.svalue.len_used, pweb->output_val.svalue.len_alloc, (int) out.length, hsize);
   mg_log_event(pweb->plog, pweb, bufferx, "mg_web_execute: YottaDB API", 0);
#if 0
   mg_log_buffer(pweb->plog, pweb, out.address, out.length, "mg_web_execute: YottaDB API: value", 0);
#endif
}
*/
...
*/
         ...
         *ugroup = '\0';
         ...
         sprintf(buffer, "Error invoking YottaDB function %s; result: %d", fun.label, rc);
         mg_log_event(pweb->plog, pweb, buffer, "mg_web_execute: YottaDB API error", 0);
      }
